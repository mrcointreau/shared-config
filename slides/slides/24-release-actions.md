...
<!--
The release sequence is: get bot token, run semantic-release, publish to registry if enabled, then create tag. Publish happens before tag for atomicity—if publish fails, you don't get a dangling Git tag pointing to a version that doesn't exist in the registry. The publish actions can also be used standalone for recovery; let's look at those next.
-->
